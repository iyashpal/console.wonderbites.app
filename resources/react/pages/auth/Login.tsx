  ...
  return <>
    <GuestLayout>
      <AuthCard>
        ...
        <Form onSubmit={form.onSubmit.login} method='post' className='space-y-6 w-full'>
          ...
          <div className="flex flex-col items-center w-full pt-3 space-y-4">
            <button disabled={form.isProcessing} className="bg-red-primary py-3 px-5 block rounded uppercase font-semibold text-white w-full hover:bg-opacity-60 transition-all ease-in-out duration-700 disabled:bg-opacity-40">
              {form.isProcessing ? 'Processing...' : 'Log In'}
            </button>

            <Link to={'/forgot-password'} className="text-red-primary hover:underline transition-all ease-in-out duration-700">
              Forgot Password?
            </Link>
          </div>
        </Form>
