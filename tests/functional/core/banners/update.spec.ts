...
test.group('Core [banners.update]', (group) => {
  ...
  test('it reads 401 status code when user is unauthenticated.', async ({client, route}) => {
    ...
    const response = await client.put(route('core.banners.update', banner))

    response.assertStatus(401)
    response.assertBodyContains({message: 'Unauthorized access'})
  }).tags(['@core', '@core.banners', '@core.banners.update'])

  test('it reads 401 status code when user is authenticated with invalid role.', async ({client, route}) => {
    const banner = await BannerFactory.create()
    const user = await UserFactory.create()
    const response = await client.put(route('core.banners.update', banner)).guard('api').loginAs(user)
